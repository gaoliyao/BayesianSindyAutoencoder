...
from autoencoder_constraints import add_constraints
...
def train_network(training_data, val_data, params):
    ...
    if params['constrained']:
        autoencoder_network, loss, losses, loss_refinement = add_constraints(autoencoder_network, loss, losses, loss_refinement)
    ...
    losses_for_ratio = (losses['decoder'], losses['sindy_x'], losses['sindy_z'])
    loss_ratio_bases = (x_norm, sindy_predict_norm_x, 1.0)
    if params['include_true_dynamics']:
        losses_for_ratio += (losses['true_dynamics'],)
        loss_ratio_bases += (np.mean(val_data['z']**2),)
    ...
    with tf.Session() as sess:
        ...
        for i in range(params['max_epochs']):
            ...
            if params['print_progress'] and (i % params['print_frequency'] == 0):
                ...
                validation_losses.append(print_progress(sess, i, loss, losses, losses_for_ratio, loss_ratio_bases, train_dict, validation_dict))
...
#                validation_losses.append(print_progress(sess, i_refinement, loss_refinement, losses, train_dict, validation_dict, x_norm, sindy_predict_norm_x))
                validation_losses.append(print_progress(sess, i_refinement, loss_refinement, losses, losses_for_ratio, loss_ratio_bases, train_dict, validation_dict))
        ...
        if params['fixed_coefficients']:
            sindy_coefficients = val_data['sindy_coefficients']
        else:
...
def print_progress(sess, i, total_loss, losses, losses_for_ratio, ratio_bases, train_dict, validation_dict):
    """
    ...
    """
    training_loss_vals = sess.run((total_loss,) + tuple(losses.values()), feed_dict=train_dict)
    validation_loss_vals = sess.run((total_loss,) + tuple(losses.values()), feed_dict=validation_dict)
    print("Epoch %d" % i)
    ...
    loss_ratio_vals = sess.run(losses_for_ratio, feed_dict=validation_dict)
    ratios = [loss_ratio_vals[i]/ratio_bases[i] for i in range(len(ratio_bases))]
    print('loss ratios: {0}'.format(ratios))
...
def create_feed_dictionary(data, params, idxs=None):
    ...
    if params['fixed_coefficients']:
        feed_dict['sindy_coefficients:0'] = data['sindy_coefficients']
    if params['include_true_dynamics']:
        feed_dict['latent_dynamics:0'] = data['z'][idxs]
