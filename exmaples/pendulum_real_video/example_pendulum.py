...
#     WH = 27 * 24 # 72 * 108 # 32*50
#     data_train = {}
#     data_test = {}
#     data_train['t'] = np.load("single_pendulum_t.npy")
#     data_train['x'] = np.load("x_gaussian.npy")
#     data_train['x'] = data_train['x'].reshape(-1, WH)
#     length_x = len(data_train['x'])
#     t_train = int(length_x * 0.98)
#     data_train['t'] = np.load("single_pendulum_t.npy")[:t_train]
#     data_train['x'] = data_train['x'][:t_train]
#     data_train['dx'] = np.load("dx_gaussian.npy").reshape(-1, WH)
#     data_train['dx'] = data_train['dx'][:t_train]
#     data_train['ddx'] = np.load("ddx_gaussian.npy").reshape(-1, WH)
#     data_train['ddx'] = data_train['ddx'][:t_train]
    
#     data_test['t'] = np.load("single_pendulum_t.npy")[t_train:]
#     data_test['x'] = np.load("x_gaussian.npy").reshape(-1, WH)
#     data_test['x'] = data_test['x'][t_train:]
#     data_test['dx'] = np.load("dx_gaussian.npy").reshape(-1, WH)
#     data_test['dx'] = data_test['dx'][t_train:]
#     data_test['ddx'] = np.load("ddx_gaussian.npy").reshape(-1, WH)
#     data_test['ddx'] = data_test['ddx'][t_train:]
#     print("data_test['x'].shape", data_test['x'].shape)
#     print("data_test['dx'].shape", data_test['dx'].shape)
#     print("data_test['ddx'].shape", data_test['ddx'].shape)
...
def load_pendulum_data():
    ...
    data_train = {}
    data_test = {}
    data_train['t'] = np.load("single_pendulum_t.npy")[:length]
    data_train['x'] = np.load("x_gaussian.npy")
    data_train['x'] = data_train['x'].reshape(-1, WH)[:length]
    length_x = len(data_train['x'])
    t_train = int(length_x * 0.98)
    data_train['t'] = np.load("single_pendulum_t.npy")[:t_train]
    data_train['x'] = data_train['x'][:t_train]
    ...
    data_test['t'] = np.load("single_pendulum_t.npy")[t_train:length]
    data_test['x'] = np.load("x_gaussian.npy").reshape(-1, WH)
    data_test['x'] = data_test['x'][t_train:length]
    data_test['dx'] = np.load("dx_gaussian.npy").reshape(-1, WH)
    data_test['dx'] = data_test['dx'][t_train:length]
    data_test['ddx'] = np.load("ddx_gaussian.npy").reshape(-1, WH)
    data_test['ddx'] = data_test['ddx'][t_train:length]
    print("data_test['x'].shape", data_test['x'].shape)
    print("data_test['dx'].shape", data_test['dx'].shape)
    print("data_test['ddx'].shape", data_test['ddx'].shape)
